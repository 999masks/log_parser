...
if need_subtest == False : # wil cover test case dot subtest feauture,
  for line_1 in temp_result:
    raw_test_name = re.search("^/.*?/results-\d+-([^/ \t]+)", line_1)
  ...
  for line_2 in temp_result:
    ...
    try:
      if len(line_2) > 1 and "[" in line_2:
        raw_test_name = re.search("^/\S+results-\d+-(\S+).*\[\s+(PASSED|FAILED|ERROR)", line_2) # ? mark at the end of brackets is indicating optional group
        print line_2
        test_name = raw_test_name.group(1)
        #test_status = raw_test_name.group(2)
      ...
      elif len(line_2) > 1 and line_2.find("[")== -1 :
        raw_test_name = re.search("^/\S+results-\d+-(\S+)\s+(.*[^\n])", line_2) # if there is no square bracket, it is error log  ? mark at the end of brackets is indicating optional group
        test_log = raw_test_name.group(2)
    except:
      print "exept", line_2



    #print "TEST_name", test_name #, "TEST_status", test_status, "TEST_log", test_log
